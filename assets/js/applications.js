...
function getDetailMyEndowment(id) {
  ...
  endowments.fetch({
    success: function(response, xhr) {
      window.my_detail_endowment = JSON.parse(JSON.stringify(response));

      var my_endowment = window.my_detail_endowment[0].endowment.endowment;
...
function memberCharityEndowment(data, id) {
  ...
  if (data.length == 0) {
    ...
    $.each(data, function(key, val) {
      ...
      $('#member-charities-my-endowment-' + id).append("<ul style='list-style: none; margin: auto;'><li>Member Charity "+ (key+1) + "<br /><div style='margin-left: 15px;'><div style='float: left;'>Name: </div><div style='margin-left: 45px;'>" + val.name +"</div></div><div style='margin-left: 15px;'><div style='float: left;'>State: </div><div style='margin-left: 45px;'>-</div></div><div style='margin-left: 15px;'><div style='float: left;'>City: </div><div style='margin-left: 45px;'>-</div></div></li><br /></ul>");
    })
}
}
            ...
            if (str.indexOf("new_endowment") !== -1) {
              ...
              $.each(window.charities, function(key, val) {
                if (val.message !== "Not found") {
                  ...
                  $('#charity-details').append("<div id='charity-details-"+ val.charity.id +"'><p>Charity Name: <b>"+ val.charity.name +"</b></p><p>Charity Email: <b>"+ val.charity.email +"</b></p><p>Address: <b>"+ val.charity.address +"</b></p><p>City, State, Zip: <b>"+ val.charity.city + ', ' + val.charity.state + ', ' + val.charity.zip +"</b></p><p>Activity: <b>"+ val.charity.activity_code +"</b></p><p>Classification: <b>"+ val.charity.classification_code +"</b></p><p>NTEE: <b>"+ val.charity.ntee_code +"</b></p><br/><p>Total granted to charity from me: <b>"+ '-' +"</b></p><p>Total granted to charity from giv2giv: <b>"+ '-' +"</b><br/></div>");
                }
              });
}
},
